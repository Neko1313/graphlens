...
``scip-dotnet index``. Driving that batch index instead of an interactive
Roslyn LSP server is what keeps resolution complete on large solutions — the
index is written once and read back statically, with no per-query round-trip
or live-workspace timeout to go quiet on.

This is a verbatim copy of ``graphlens_rust._scip`` — the wire format it
decodes is generic SCIP, not Rust-specific, so the logic is identical. Kept as
a separate copy per package (rather than a shared core utility) to keep this
adapter's changes self-contained; consolidating the two into one
``graphlens.utils`` module is a reasonable follow-up if a third SCIP-backed
adapter shows up.
