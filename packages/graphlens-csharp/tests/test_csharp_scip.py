...
from graphlens_csharp._scip import (
...
def test_decodes_packed_range_symbol_and_roles():
    ...
    assert len(docs) == 1
    rel, occs = docs[0]
    assert rel == "src/Util.cs"
    assert occs == [
        ScipOccurrence(
            "scip-dotnet nuget . . Util#Helper().", SCIP_ROLE_DEFINITION, 3, 4
        )
    ]
...
def test_decodes_unpacked_range():
    idx = _len_field(
        2,
        _document(
            "A.cs", [_occurrence("s", 0, [1, 2, 3], packed=False)]
        ),
    )
...
def test_multibyte_varint_position():
    ...
    idx = _len_field(
        2, _document("A.cs", [_occurrence("s", 0, [200, 300, 305])])
    )
    (_rel, occs) = next(iter(iter_documents(idx)))
...
def test_occurrence_without_range_is_dropped():
    idx = _len_field(2, _document("A.cs", [_occurrence("s", 0, [5])]))
    (_rel, occs) = next(iter(iter_documents(idx)))
    assert occs == []
...
def test_multiple_documents():
    idx = _len_field(
        2, _document("A.cs", [_occurrence("x", 0, [0, 0, 1])])
    ) + _len_field(2, _document("B.cs", [_occurrence("y", 0, [1, 0, 1])]))
    paths = [rel for rel, _ in iter_documents(idx)]
    assert paths == ["A.cs", "B.cs"]


def test_skips_unknown_field_in_document():
    # A field inside a Document message that is neither relative_path (1)
    # nor occurrences (2) must be skipped without disturbing parsing.
    doc_body = (
        _varint_field(9, 3)
        + _len_field(1, b"A.cs")
        + _len_field(2, _occurrence("x", 0, [0, 0, 1]))
    )
    idx = _len_field(2, doc_body)
    docs = list(iter_documents(idx))
    assert len(docs) == 1
    rel, occs = docs[0]
    assert rel == "A.cs"
    assert occs == [ScipOccurrence("x", 0, 0, 0)]
...
def test_skips_fixed32_and_fixed64_index_fields():
    ...
    idx = (
        fixed64
        + _len_field(2, _document("A.cs", [_occurrence("x", 0, [0, 0, 1])]))
        + fixed32
    )
    docs = list(iter_documents(idx))
    assert [rel for rel, _ in docs] == ["A.cs"]
...
def test_range_field_with_unexpected_wire_type_is_ignored():
    ...
    idx = _len_field(2, _document("A.cs", [occ_body]))
    (_rel, occs) = next(iter(iter_documents(idx)))
