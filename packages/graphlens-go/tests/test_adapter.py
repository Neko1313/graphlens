...
def test_resolve_read_emits_references_with_access_metadata():
    from graphlens import RelationKind

    g = _resolution_graph()
    ref = _ref("internal", file_path=Path("a.go"), line=5, col=6)
    _resolve(g, _FakeResolver(ref), [_occ(role="read")])
    refs = [r for r in g.relations if r.kind == RelationKind.REFERENCES]
    assert len(refs) == 1
    assert refs[0].source_id == "caller"
    assert refs[0].target_id == "callee"
